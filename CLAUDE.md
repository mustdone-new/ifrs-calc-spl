...
IFRS 17 actuarial valuation engine that processes Groups of Contracts through GMM/PAA building-block calculations and exports a 380+ row audit-trail Excel workbook with CSM roll-forwards, journal entries, and LRC positions for Insurance and Reinsurance modes.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "207344382+mustdone-new@users.noreply.github.com"
  git config user.name "mustdone-new"
  ```
